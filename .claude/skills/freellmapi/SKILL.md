...
## Переезд на другой компьютер/сервер (Windows, десктоп-приложение)

Ключи провайдеров в `freeapi.db` зашифрованы ключом из файла
`.encryption-key`, который лежит **рядом с базой** в `%APPDATA%\FreeLLMAPI\`
(`server/src/lib/crypto.ts`, `KEY_FILE_NAME`). Документация
(`docs/en/install/01-install.md`) советует копировать `freeapi.db` и
`config.json` — без `.encryption-key` ключи на новой машине не расшифруются,
поэтому копировать нужно **всю папку**. SQLite копировать только при
закрытом приложении.

Готовые скрипты в `scripts/`:
- `backup-for-move.cmd` — закрывает FreeLLMAPI и кладёт на Рабочий стол
  `move-backup-<дата>.zip`: папку `%APPDATA%\FreeLLMAPI` целиком,
  `~\.claude` и `~\.claude.json` (настройки и локальные сессии Claude Code),
  папку со скриптами и, по желанию, Рабочий стол и Документы. В архиве секреты.
- `restore-after-move.cmd` — на новой машине (после установки FreeLLMAPI и
  Node.js) раскладывает архив по местам; всё, что заменяется, откладывает в
  `*.before-restore-<время>`, а файлы на Рабочем столе и в Документах не
  перезаписывает.

Локальные сессии Claude Code привязаны к пути папки (`~\.claude\projects\…`):
`/resume` найдёт их, только если на новой машине тот же путь (то же имя
пользователя). Облачные сессии claude.ai и репозитории GitHub от сервера не
зависят.
